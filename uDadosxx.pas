...
uses
  ...
  Parametros, uLogin, Variants, uCodifica,  uAguarde, ComCtrls,
  StrUtils,
  uFreeReport, uGetFunctions,
  uFuncoes, kbmMemTable;

Type  TSituacaoCobranca = (tsitPendente, tsitAcordado, tsitEncerrado);


type
  TDadosxx = class(TDataModule)
    ...
    MT_PENDENCIAS_CREDOR: TkbmMemTable;
    MT_PENDENCIAS_CREDORCREDOR: TIntegerField;
    MT_PENDENCIAS_CREDORNOME_CREDOR: TStringField;
    MT_PENDENCIAS_CREDORDEVEDOR: TIntegerField;
    MT_PENDENCIAS_CREDORNOME_DEVEDOR: TStringField;
    MT_PENDENCIAS_CREDORDATA_DIVIDA: TDateField;
    MT_PENDENCIAS_CREDORVENCIMENTO: TDateField;
    MT_PENDENCIAS_CREDORVALOR: TFloatField;
...
function TDadosxx.flRetornarProximaSequencia: Integer;
var
  FQuery : TADOQuery;
  int : integer;
begin
...
procedure TDadosxx.plAtualizarTotal;
var
  vQry     : TADOQuery;
  Cobranca : Integer;
  Soma     : Double;
...
procedure TDadosxx.TB_ACIONAMENTOBeforePost(DataSet: TDataSet);
begin
  Try
...
procedure TDadosxx.TB_ITENS_COBRANCAAfterInsert(DataSet: TDataSet);
begin
  TB_ITENS_COBRANCAITEM.Value := flRetornarProximaSequencia;
end;

procedure TDadosxx.TB_ITENS_COBRANCAAfterPost(DataSet: TDataSet);
begin
  plAtualizarTotal;
end;

procedure TDadosxx.TB_COBRANCABeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_ITENS_COBRANCABeforePost(DataSet: TDataSet);
begin
  Try
...
procedure TDadosxx.TB_ITENS_COBRANCAEMISSAO_DOCUMENTOChange(Sender: TField);
begin
  TB_ITENS_COBRANCAVENCIMENTO_DOCUMENTO.AsDateTime := TB_ITENS_COBRANCAEMISSAO_DOCUMENTO.AsDateTime;
end;


function TDadosxx.plParametrosDataBase: String;
begin
   Result := 'Provider=Microsoft.Jet.OLEDB.4.0;'+
             'User ID=Admin;'+
             'Data Source='+TUniqueParametros.GetParametros.DatabaseName+'wCob.mdb;'+
             'Mode=Share Deny None;'+
             'Extended Properties="";'+
             'Jet OLEDB:System database="";'+
             'Jet OLEDB:Registry Path="";'+
             'Jet OLEDB:Database Password="WIGCRED38";'+
...
procedure TDadosxx.plConectarUsuario;
Var
  UsuarioOK : Boolean;
begin
  Repeat
    ...
    If   Not TUniqueParametros.GetParametros.CancelaLogin Then
         Begin
           If  Not TB_USUARIO.Locate('USUARIO;SENHA', VarArrayOf([TUniqueParametros.GetParametros.Usuario,
                                                                  TUniqueParametros.GetParametros.Senha]), [loPartialKey]) Then
               ShowMessage('Usuario não Cadastrado')
           Else
              Begin
  {$IFNDEF DEBUG}
                If  TB_USUARIOUSUARIO_LOGADO.Value Then
                    ShowMessage('Usuario Logado');
  {$ENDIF}
                TB_USUARIO.Edit;
                TB_USUARIOENDERECO_IP.Value     := TUniqueParametros.GetParametros.IPNumber;
                TB_USUARIONOME_COMPUTADOR.Value := TUniqueParametros.GetParametros.HostName;
                TB_USUARIOUSUARIO_REDE.Value    := TUniqueParametros.GetParametros.UsuarioSistema;
                TB_USUARIOUSUARIO_LOGADO.Value  := True;
                TB_USUARIO.Post;

                UsuarioOK := True;

              End;
              TUniqueParametros.GetParametros.Privilegiado := TB_USUARIOTIPO_USUARIO.Value = True;
              TUniqueParametros.GetParametros.EhCobrador   := False;

              If   TB_COBRADOR.Locate('IDUSUARIO',TB_USUARIOID.Value,[]) Then
                   Begin
                     TUniqueParametros.GetParametros.EhCobrador := True;
                     TUniqueParametros.GetParametros.IDCobrador := TB_COBRADORIDCOBRADOR.Value;
                   End;
         End;
  Until TUniqueParametros.GetParametros.CancelaLogin or UsuarioOK;
end;

procedure TDadosxx.TB_USUARIOSENHASetText(Sender: TField;
  const Text: String);
begin
  TB_USUARIOSENHA.Value := Criptografar(Text);
end;

procedure TDadosxx.TB_ITENS_COBRANCACalcFields(DataSet: TDataSet);
begin
...
procedure TDadosxx.plParametrosSistema;
begin
  TB_PARAMETROS.First;
...
procedure TDadosxx.TB_ALINESBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_CREDORBeforePost(DataSet: TDataSet);
begin
  Try
...
procedure TDadosxx.TB_RECEBIMENTOBeforePost(DataSet: TDataSet);
begin
  Try
...
procedure TDadosxx.TB_ESTADOBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_DEVEDORBeforePost(DataSet: TDataSet);
begin
  Try
...
procedure TDadosxx.TB_USUARIOBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_TIPO_DOCUMENTOBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_TELEFONESBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_TIPO_PESSOABeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_TIPO_SITUACAOBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_BANCOBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_ENDERECOSBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.plConsultaCobrancaporDevedor(const ciDevedor : Integer);
begin
...
procedure TDadosxx.TB_ITENS_COBRANCAAfterDelete(DataSet: TDataSet);
begin
  plAtualizarTotal;
end;

procedure TDadosxx.plConsultaAcionamentoporDevedor(const ciDevedor : Integer);
begin
...
procedure TDadosxx.plConsultaAcionamentoporCobranca(const ciCobranca : Integer);
begin
  With QU_CONSULTA_ACIONAMENTO Do
    Begin
...
procedure TDadosxx.plRetornaItensMenu(IdMenuPai: String);
begin
  With QU_MENU Do
    Begin
...
function TDadosxx.plLiberaAcessoMenu(Menu: String) : Boolean;
begin
  Try
...
procedure TDadosxx.plConsultaParcelas(const ciCobranca : Integer);
begin
...
procedure TDadosxx.plAtualizarAcordo;
var
  vQry       : TADOQuery;
  Achou      : Boolean;
  lbParcelas : Byte;
...
procedure TDadosxx.TB_RECEBIMENTOQUITADOValidate(Sender: TField);
begin
...
procedure TDadosxx.TB_RECEBIMENTOCalcFields(DataSet: TDataSet);
begin
  If   TB_RECEBIMENTOPAGO.Value Then
...
procedure TDadosxx.plAbrirTabelas;
Var
  I : Integer;
begin
...
procedure TDadosxx.plFecharTabelas;
Var
  I : Integer;
begin
  For I := 0 to ComponentCount - 1 do
...
procedure TDadosxx.DataModuleDestroy(Sender: TObject);
begin
  plFecharTabelas;
end;

function TDadosxx.flRetornarQuantRecebimentoRepasse: Integer;
var
  FQuery : TADOQuery;
begin
  FQuery := TADOQuery.Create(Self);
...
function TDadosxx.flRetornarValorTotalRecebimentoRepasse: Double;
var
  FQuery : TADOQuery;
begin
  FQuery := TADOQuery.Create(Self);
...
procedure TDadosxx.plConsultaItensCobranca(const ciCobranca : Integer);
begin
...
procedure TDadosxx.plConsultaCobrancaporDevedorSintetico(const ciDevedor :
    Integer);
begin
  With QU_CONSULTACOBRANCA Do
    Begin
...
procedure TDadosxx.plConsultaAcordoDevedorSintetico(const ciDevedor : Integer);
begin
...
procedure TDadosxx.plConsultaCredor(const ciCredor : Integer);
begin
...
procedure TDadosxx.plConsultaDevedorTelefones(const ciDevedor : Integer);
begin
...
procedure TDadosxx.plConsultaDevedorEndereco(const ciDevedor : Integer);
begin
...
procedure TDadosxx.plConsultaItensAcordo(const ciCobranca : Integer);
begin
...
procedure TDadosxx.TB_DEVEDORCNPJ_CPFValidate(Sender: TField);
begin
  If  Trim(Sender.Value) = '' Then
      Raise Exception.Create('Preenchimento do Campo é Obrigatório!');
end;

procedure TDadosxx.TB_ACORDOBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_BOLETOBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_COBRADORBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_DEPREBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_GRUPO_COBRANCABeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_MENUBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_METASBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_MOTIVOS_ACIONAMENTOSBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_PARAMETROSBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_RECIBOBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_REPASSEBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.TB_USUARIO_MENUBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
...
procedure TDadosxx.plDesconectarUsuario;
begin
...
procedure TDadosxx.plPosicionaCredor(cCredor : Integer);
begin
  Dados.TB_CREDOR.Locate('CODIGO_CREDOR',cCredor,[]);
end;

procedure TDadosxx.plPosicionaDevedor(cDevedor : Integer);
begin
  Dados.TB_DEVEDOR.Locate('CODIGO_DEVEDOR',cDevedor,[]);
end;

procedure TDadosxx.plResumoCredor(cCredor: Integer; var cAberto, cAcordo,
  cFechado: Double);
var
  FQuery : TADOQuery;
begin
...
function TDadosxx.flRetornaQuantidadeQuitadas(cCredor: Integer): Double;
var
  FQuery : TADOQuery;
begin
  FQuery := TADOQuery.Create(Self);
...
function TDadosxx.flRetornaQuantidadeAtrasadas(cCredor : Integer): Double;
var
  FQuery : TADOQuery;
begin
  FQuery := TADOQuery.Create(Self);
...
function TDadosxx.flRetornaQuantidadeAcionadas(cCredor : Integer; cAcionadas :
    Boolean): Integer;
var
  FQuery : TADOQuery;
begin
...
procedure TDadosxx.TB_CHAMADOBeforePost(DataSet: TDataSet);
begin
  Try
    Case DataSet.State of
      dsInsert:  Begin
