...
type
  TFrmAtualizaFinanceiro = class(TForm)
    Label1: TLabel;
...
var
  FrmAtualizaFinanceiro: TFrmAtualizaFinanceiro;
...
const
    WIGCRED_EXE    = 'WIGCRED.EXE';
    FINANCEIRO_EXE = 'FINANCEIRO.EXE';
...
const
  CHAVEREG = 'Software\WigCred\Atualizador WigCred';
...
procedure TFrmAtualizaFinanceiro.DisposeStrList(const StrList: TStrings);
var
  I: Integer;
begin
...
function TFrmAtualizaFinanceiro.GetRegDir: String;
var
  FReg: TRegIniFile;
  F: TextFile;
begin
  Try
    ...
    begin
      ...
      raise Exception.Create('Falta o arquivo de configuração. Execute o DefineDir');
  Except
    On E:Exception Do
    Begin
...
procedure TFrmAtualizaFinanceiro.GetExtNames(const ExtDir, ExtName: String; const StrLst: TStrings);
var
  SR: TSearchRec;
  InfoFile: TInfoFile;
begin
...
procedure TFrmAtualizaFinanceiro.GetExeNames(const ExeDir: String; const StrLst: TStrings);
begin
  StrLst.BeginUpdate;
  try
    GetExtNames(ExeDir, FINANCEIRO_EXE, StrLst);
  finally
    StrLst.EndUpdate;
  end;
end;

procedure TFrmAtualizaFinanceiro.GetININames(const ExeDir: String;
  const StrLst: TStrings);
begin
  StrLst.BeginUpdate;
  try
    GetExtNames(ExeDir, ChangeFileExt(WIGCRED_EXE, '.UDL'), StrLst);
  finally
    StrLst.EndUpdate;
  end;
end;

procedure TFrmAtualizaFinanceiro.UpdateFile(FileName: String);
var
  OldName: String;
  SRLocal: TSearchRec;
  Ext: String;
...
procedure TFrmAtualizaFinanceiro.CompareCopyFiles(const SourceDir, DestDir: String; const FStrList: TStrings; const FOverWrite : Boolean = True);
var
  x: Integer;
  SRLocal: TSearchRec;
  L,R: TDateTime;
                    ...
                    if (MinutesBetween(L, R) > 2) or
                       ...
                       begin
                         ...
                         try
                           ...
                           if not CopyFile(pChar(SourceDir + FStrList[x]), pChar(DestDir + FStrList[x]), FALSE) then
                              Exception.CreateFmt('%s - %d' , [SysErrorMessage(GetLastError), GetLastError]);
                           if  CompareText(FStrList[x], 'DefineDir.exe') = 0 then
                           begin
                              WinExec('DefineDir.exe', SW_SHOWNORMAL);
                              Halt;
                           end;
                         end;
                       end;
...
{Fim - Dalutex - Andre - 13/07/2005 - Copiando Arquivos de Configuração}
      FindClose(SRLocal);
    end;
  except
    raise;
...
procedure TFrmAtualizaFinanceiro.Atualiza;
var
  Indice : LongInt;
  StrList: TStrings;
  LocalP,
...
procedure TFrmAtualizaFinanceiro.TimerAtualizaTimer(Sender: TObject);
begin
...
function TFrmAtualizaFinanceiro.flEhAcessoLocal: Boolean;
Var
  lsPastaAtual : String;
begin
